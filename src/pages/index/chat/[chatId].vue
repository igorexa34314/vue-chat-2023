<template>
	<v-container class="h-100 pa-0 d-flex flex-column" fluid>
		...
		<div
			v-if="userStore.chats.length && !userStore.chats.some(el => el.info.id === chatId)"
		...
		<!-- Chat content -->
		<div
			v-else-if="userStore.chats && userStore.chats.length"
			class="chat__field flex-fill overflow-hidden d-flex flex-column flex-grow-1">
			<div class="chat__content w-100 flex-fill">
				<div v-if="isChatLoading || messagesStore.isLoadingFirst"><PageLoader /></div>
				<Suspense v-else-if="messagesStore.messages.length" @resolve="handleSuspenceResolve">
					<v-infinite-scroll
						...
						<div
							class="messages-field flex-fill w-100 d-flex flex-column justify-end px-1 px-sm-3 my-0 mx-auto">
							<v-slide-y-reverse-transition group hide-on-leave :disabled="!allowTransition">
								<MessageItem
									v-for="m in messagesStore.messages"
									:key="m.id"
									:self="userStore.info?.uid === m.sender.uid"
									:content="m.content"
									:sender="m.sender"
									...
									:selected="m.id === msgCtxMenu.activeMessage?.id"
							...
							<ContextMenu
								...
								:self="userStore.info?.uid === msgCtxMenu.activeMessage?.sender.uid"
								:max-width="350"
								:min-width="xs ? 0 : 230"
								:position="msgCtxMenu.position"
								:scroll-container="scrollRef ?? undefined"
								:is-selected="!!selectedText"
								@closed="ctxMenuClosed"
								@copy-selected-text="copySelectedText"
			...
			<MessageForm
				...
				ref="msgForm"
				@create-message="tryCreateMessage"
				@update-message="tryUpdateMessage"
			...
			<v-fade-transition>
				<v-btn
					v-if="
						scrollRef && scrollRef.scrollHeight > scrollRef.clientHeight && !isScrollOnBottom
					"
...
import { getChatInfoById, type ChatInfo } from '@/services/chat';
import { ref, computed, onUnmounted, nextTick, watch, toRef, useTemplateRef } from 'vue';
...
const {
	...
	async (id?: ChatInfo['id']) => {
		const chatInfo = await getChatInfoById(id ?? chatId.value);
		if (chatInfo) {
			title.value = setChatName.value(chatInfo);
			emit('updateTitle', title.value);
		}
		const unsub = subscribeMessages(id ?? chatId.value) as Unsubscribe | null;
		await fetchMessages(id ?? chatId.value);
...
const vuetifyScrollRef = useTemplateRef<VInfiniteScroll>('vuetifyScrollEl');
const scrollRef = toRef(() => vuetifyScrollRef.value?.$el as HTMLElement | null);

const { isScrollOnBottom, scrollBottom, onLoad, scrollSide } = useChatScroll(
	scrollRef,
	async direction => {
		allowTransition.value = false;
		await loadMoreMessages(chatId.value, direction);
...
const tryCreateMessage = async <T extends ContentType>(content: CreateMsgForm<T>) => {
	try {
		await createMessage(chatId.value, content);
	} catch (e) {
		showMessage(sbMessages[e as keyof typeof sbMessages] || (e as string), 'red-darken-3', 2000);
	}
};
const tryUpdateMessage = async ({ id, content }: EditMessageData) => {
	try {
		await updateMessage(chatId.value, { id, content });
	} catch (e) {
		showMessage(sbMessages[e as keyof typeof sbMessages] || (e as string), 'red-darken-3', 2000);
	}
};
...
const msgCtxMenu = ref<MessageContextMenu>({
	...
	position: { x: 0, y: 0 },
	activeMessage: null,
	closestAttachementId: null,
});

useEventListener(
	() => (msgCtxMenu.value.show ? scrollRef.value : null),
	['wheel', 'touchmove'],
	event => {
...
const openCtxMenu = (e: MouseEvent, message: Message) => {
	...
	if (message.content.type !== 'text') {
		msgCtxMenu.value.closestAttachementId =
			(e.target as HTMLElement | null)
				?.closest(
					msgCtxMenu.value.activeMessage?.content.type === 'media'
						? '.image__wrapper'
						: '.file__wrapper'
				)
				?.getAttribute('data-attachment-id') ?? null;
	}
...
const getAllMedia = computed<MediaAttachment[]>(() =>
	messagesStore.messages
		...
		.flatMap(m =>
			(m.content as MessageContent<'media'>).attachments.filter(f => f.raw?.previewURL)
		)
);
...
const openInOverlay = (imgId: MediaAttachment['id']) => {
	overlayState.value = {
		show: true,
		currentImage: getAllMedia.value?.findIndex(img => img.id == imgId),
	};
};
const overlayClosed = () => {};
...
const copySelectedText = async () => {
	try {
		await copyToClipboard(selectedText.value.trim());
...
const copyImage = async () => {
	try {
		...
		const imageToCopy =
			msgContent?.attachments.find(attach => attach.id === msgCtxMenu.value.closestAttachementId)
				?.raw.previewURL ||
...
const msgFormRef = useTemplateRef('msgForm');
const editMessage = () => {
	if (msgCtxMenu.value.activeMessage?.id) {
		msgFormRef.value?.editMessage(msgCtxMenu.value.activeMessage);
	}
};

const showConfirmation = ref(false);
const tryDeleteMessage = async () => {
	if (msgCtxMenu.value.activeMessage?.id) {
		await deleteMessageById(chatId.value, msgCtxMenu.value.activeMessage?.id);
	}
};

// Hightlight message
const goTo = useGoTo();
const highlightedMessageId = refAutoReset('', 600);

const scrollToAndHighlightMessage = async (mId: Message['id']) => {
	// if (!messages.some(m => m.id == mId)) {
	// 	focusOnMessage(mId);
	// 	await nextTick();
	// }
	// Scroll to the message
	await goTo(`[data-message-id="${mId}"]`, {
		container: scrollRef.value ?? undefined,
		offset: -80,
	});
...
const { isOverDropZone } = useDropZone(
	toRef(() => scrollRef.value),
	files => {
		if (files) {
			msgFormRef.value?.attachFiles('file', files);
		}
	}
);
