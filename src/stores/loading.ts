...
export const useLoadingStore = defineStore('loading', () => {
	const uploadingState = shallowRef<UploadingState[]>([]);

	const getUploadingStateById = computed(
		() => (id: string) => uploadingState.value.find(l => l.fileId === id)
	);
	...
	const finishLoading = (id: string) => {
		uploadingState.value = uploadingState.value.map(l =>
			l.fileId === id ? { ...l, status: 'finished' } : l
		);
	};
	...
	return {
		uploadingState: readonly(uploadingState),
